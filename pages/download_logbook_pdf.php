...
// FILTER LOGIC
// =======================
$filter = $_GET['filter_type'] ?? 'all';
$report_type = $_GET['report_type'] ?? 'borrowed';
$where = "";
$returned_where = "";
...
elseif($filter == "month"){
    $where = "MONTH(release_date) = MONTH(CURDATE()) AND YEAR(release_date) = YEAR(CURDATE())";
    $returned_where = "MONTH(return_date) = MONTH(CURDATE()) AND YEAR(return_date) = YEAR(CURDATE())";
}
elseif($filter == "year"){
    $where = "YEAR(release_date) = YEAR(CURDATE())";
    $returned_where = "YEAR(return_date) = YEAR(CURDATE())";
}
...
if($filter == "today"){
    $period_text = "Date: " . date("F d, Y");
} elseif($filter == "week"){
    $period_text = "Period: Last 7 Days";
} elseif($filter == "month"){
    $period_text = "Period: " . date("F Y");
} elseif($filter == "year"){
    $period_text = "Period: Year " . date("Y");
} elseif($filter == "custom" && isset($start, $end)){
    $period_text = "Period: " . date("M d, Y", strtotime($start)) . " - " . date("M d, Y", strtotime($end));
}

$title_text = ($report_type == 'returned') ? "Returned Equipment Records" : "Equipment Borrowing Logbook";

$html = '
<h2 style="text-align:center;">DEPARTMENT OF AGRICULTURE</h2>
<h4 style="text-align:center;">'.$title_text.'</h4>
<p style="text-align:center; font-size: 10pt;">'.$period_text.'</p>
<br>
';

// =======================
// BORROWED RECORDS (DEFAULT)
// =======================
...
if ($report_type == 'returned') {
    $returned_sql = "SELECT * FROM borrow_records WHERE status IN ('RETURNED', 'LOST', 'DAMAGED')";
...
<th width="55"><b>Returned By</b></th>
...
';
    ...
    if($returned_result && $returned_result->num_rows > 0){
        while($returned_row = $returned_result->fetch_assoc()){
            ...
            $returned_by = trim((string) ($returned_row['returned_by'] ?? ''));
            if ($returned_by === '') {
                $returned_by = '-';
            }
            $returned_by = htmlspecialchars($returned_by, ENT_QUOTES, 'UTF-8');
